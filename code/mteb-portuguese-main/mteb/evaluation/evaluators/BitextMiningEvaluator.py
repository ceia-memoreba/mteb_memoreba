...
class BitextMiningEvaluator(Evaluator):
    def __init__(
        self, sentences1, sentences2, gold, batch_size=32, limit=None, embed_dict=None, **kwargs
    ):
    ...
    def compute_metrics(self, model):
        ...
        for sent in sentences:
            if sent not in self.embed_dict:
                senteces_to_embed.append(sent)
        logger.info(f"Encoding {len(senteces_to_embed)} sentences...")
        embeddings = model.encode(senteces_to_embed, batch_size=self.batch_size)
        new_emb_dict = {sent: emb for sent, emb in zip(senteces_to_embed, embeddings)}
        self.embed_dict.update(new_emb_dict)
        embeddings1 = np.asarray([self.embed_dict[sent] for sent in self.sentences1])
        embeddings2 = np.asarray([self.embed_dict[sent] for sent in self.sentences2])
