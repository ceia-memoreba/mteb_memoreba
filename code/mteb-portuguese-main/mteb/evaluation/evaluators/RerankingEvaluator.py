...
class RerankingEvaluator(Evaluator):
    ...
    def compute_metrics_batched(self, model):
        ...
        for sample in self.samples:
            for positive in sample["positive"]:
                hash = sha1_hash(positive)
                original_hashes_order.append(hash)
                all_docs_dict[hash] = positive
            for negative in sample["negative"]:
                hash = sha1_hash(negative)
                original_hashes_order.append(hash)
                all_docs_dict[hash] = negative
        
        all_unique_docs = []
        all_unique_hashes = []
        for hash, doc in all_docs_dict.items():
            all_unique_docs.append(doc)
            all_unique_hashes.append(hash)

        all_unique_docs_embs = np.asarray(
            encode_corpus_func(all_unique_docs, batch_size=self.batch_size)
        )

        # Reconstruct the original order
        all_docs_embs = np.zeros((len(original_hashes_order), all_unique_docs_embs.shape[1]), dtype=all_unique_docs_embs.dtype)
        for i, hash in enumerate(original_hashes_order):
            idx = all_unique_hashes.index(hash)
            all_docs_embs[i] = all_unique_docs_embs[idx]
